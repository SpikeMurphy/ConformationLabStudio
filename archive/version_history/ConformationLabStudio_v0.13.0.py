    ...
    def run_process():
            ...
            for line in process.stdout:
                if line.strip():
                    ...
                    if "setting max_seq" in low:
                        safe_status_update("Running Multiple Sequence Alignment...", color="#00FFC3")
                    elif "model_1" in low:
                        safe_status_update("Predicting with Model 1...", color="#D2ADFF")
                    elif "model_2" in low:
                        safe_status_update("Predicting with Model 2...", color="#B67BFF")
                    elif "model_3" in low:
                        safe_status_update("Predicting with Model 3...", color="#9C4AFF")
                    elif "model_4" in low:
                        safe_status_update("Predicting with Model 4...", color="#8521FF")
                    elif "model_5" in low:
                        safe_status_update("Predicting with Model 5...", color="#7300FF")
                    elif "relax" in low:
        ...
        # stop running a task animation
            ...
            if process and process.returncode == 0:
                safe_status_update(f"✅ Completed successfully in {elapsed_str}", color="green")
                write_output(f"Run completed successfully in {elapsed_str}")
            else:
                safe_status_update(f"❌ Failed after {elapsed_str}", color="red")
                write_output(f"Run failed after {elapsed_str}")
            process = None
...
ConformationLab Studio v1.13.0 - 26.01.2026    
-----------------------------------
- ambertools added to environment

ConformationLab Studio v1.12.2 - 12.01.2026
-----------------------------------
